...
Review date: **2026-09-24**, in two passes. Environment: macOS **26.6.2** (build 25G83, Apple Silicon
...
skills) and **8 agents**, at ~1,336 tokens always present (measured with `claude plugin details` on 0.2.1). The exact procedure is in
...
agents           360-scout, chief-editor, clip-analyst, creative-director,
                 critic-reviewer, photo-curator, trend-researcher, video-builder
```
...
The 13 `.py` files pass `python3 -m py_compile`. These ten answer `--help` under `uv run`, re-checked
after every fix in this pass: `render.py`, `resources.py`, `reframe360.py`, `voice.py`, `narrate.py`,
`capcut_voice.py`, `inventory.py`, `validate_dates.py`, `export.py` and `sheets.py`. `config.py`,
`effects.py` and `sources.py` are modules: they don't run on their own, and the ones above import them.
...
| **The 8 agents** | No agent was invoked: their prompts and contracts are reviewed text, not executed | One full `/reel` run over a small folder |
